              ...
              var pa = results[0].password;
                ...
                bcrypt.compare(password1, pa, (error, result) => {
                    if(result==true){
                       ...
                       db.query("INSERT INTO Bill (id, created_ts, updated_ts, owner_id, vendor, bill_date, due_date, amount, categories, paymentStatus) VALUES ('"+id+"','"+created_ts+"','"+updated_ts+"','"+uuid+"','"+bill.vendor+"','"+bill.bill_date+"','"+bill.due_date+"','"+bill.amount_due+"','"+stringObj+"','"+bill.paymentStatus+"')",function(error,ress){
                        console.log(ress);
                        if(error){
                            console.log("error in insert");
                            throw error;
                        }
                        if(!bill.vendor || !bill.bill_date || !bill.due_date || !bill.amount_due || !bill.categories || !bill.paymentStatus){
    ...
    if (req.headers.authorization && req.headers.authorization.search('Basic ') === 0){
        ...
        db.query( `select id, first_name, last_name, password, email_address, account_created, account_updated from user_details where email_address = "${username1}"`,function(error, results,row){
            if(error){
                ...
                bcrypt.compare(password1, pa, (error, result) => {
                    if(result==true){
                        db.query(`Select id, created_ts, updated_ts, owner_id, vendor, bill_date, due_date, amount, categories from Bill where id = "${id}"`,function (error,resulte){
                            ...
                            if(resulte[0].owner_id == results[0].id){  
                                ...
                                res.status(400).json({error: "Cannot fetch other users bill"});
                            }
                        ...
                        res.send(400).json({error: "User does not exist"});
                    }
                });
            }
        });
    ...
    if (req.headers.authorization && req.headers.authorization.search('Basic ') === 0){
        ...
        db.query( `select id, first_name, last_name, password, email_address, account_created, account_updated from user_details where email_address = "${username1}"`,function(error, results,row){
            if(error){
                ...
                bcrypt.compare(password1, pa, (error, result) => {
                    if(result==true){
                        //db.query(`Select id, created_ts, updated_ts, owner_id, vendor, bill_date, due_date, amount, categories from Bill where id = "${id}"`,function (error,resulte,rows,fields){
                    ...
                    if(result==true){
                        db.query(`Select * from Bill where id = "${id}"`, function(error, ress){
                            ...
                            if(error){
                                ...
                                db.query("UPDATE Bill SET  updated_ts='"+updated_ts+"',vendor='"+bill.vendor+"',bill_date='"+bill.bill_date+"',due_date='"+bill.due_date+"',amount='"+bill.amount_due+"',categories='"+stringObj+"',paymentStatus='"+bill.paymentStatus+"' where id = '"+id+"'",function (error,resulte){
